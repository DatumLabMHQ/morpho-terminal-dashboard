...
import { loadChains, loadCurators, loadVaults } from './morpho';
...
export const searchItems: FrameData['searchItems'] = async () => {
  const [o, v, c] = await Promise.all([loadOverview(), loadVaults().catch(() => ({ vaults: [] })), loadCurators().catch(() => ({ curators: [] }))]);
  return [
    ...o.markets.map((m) => ({ label: `${m.collateral} / ${m.loan}`, href: `/markets/${m.id}`, hint: m.chain })),
    ...v.vaults.slice(0, 200).map((x) => ({ label: x.name, href: '/vaults', hint: 'Vault' })),
    ...c.curators.map((x) => ({ label: x.name, href: '/curators', hint: 'Curator' })),
  ];
};
/** The market count next to Markets in the sidebar. */
export const navBadges: FrameData['navBadges'] = async () => {
  const [o, v, c, ch] = await Promise.all([loadOverview(), loadVaults().catch(() => ({ vaults: [] })), loadCurators().catch(() => ({ curators: [] })), loadChains().catch(() => ({ chains: [] }))]);
  return { '/markets': o.markets.length, '/vaults': v.vaults.length, '/curators': c.curators.length, '/chains': ch.chains.length };
};
...
export const navChildren: FrameData['navChildren'] = async () => {
  ...
  return {
    ...
    '/curators': c.curators.map((x) => ({ label: x.name, href: '/curators' })),
    '/chains': ch.chains.map((x) => ({ label: x.chain, href: '/chains' })),
  };
};
